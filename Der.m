...
if ~isa(F,'function_handle')
    if isnumeric(F)
      xc = x(1:n-1); %chop(x); % method for data depends on how you assign dy to xc
      dy = diff(F)./diff(x); 
      varargin{1} = 'data';
    else
...
switch varargin{1}
    ...
    case 'cd'
        ...
        xc = x(1:n-1); %chop(x);
    case '3d' 
        dy = ((1/24)*F(x(1:n-1)-1.5*dx) - (27/24)*F(x(1:n-1)-0.5*dx)...
            + (27/24)*F(x(1:n-1)+0.5*dx) - (1/24)*F(x(1:n-1)+1.5*dx))./dx;  % Modification (3rd order method)
        xc = x(1:n-1);
    case 'ed'
